            ...
            <div class="flex flex-col">
                ...
                <div class="-m-1.5 overflow-x-auto">
                    ...
                    <x-modal name="createNew" :show="$errors->isNotEmpty()" focusable>
                        <form method="post" action="{{ route('voucher.claim') }}" class="p-6">
                            @csrf

                            <h2 class="text-lg font-medium text-gray-900">
                                Create New Tenant
                            </h2>
                            <x-input-label for="transaksi_id" value="{{ __('Transaksi Id') }}" class="sr-only" />

                            <x-text-input id="transaksi_id" name="transaksi_id" type="text" class="mt-4 block w-full"
                                placeholder="{{ __('Transaksi Id') }}" />
                                    ...
                                    @foreach ($transaksi as $item)
                                        <tr>
                                            <td class="h-px w-auto whitespace-nowrap">
                                                <div class="px-6 py-2 flex items-center gap-x-3">
                                                    ...
                                                    <a class="flex items-center gap-x-2" href="#">
                                                        <span
                                                            class="text-sm text-blue-600 decoration-2 hover:underline dark:text-blue-500">{{ $item->tenant->name }}</span>
                                                    </a>
                                                </div>
                                            </td>
                                            <td class="h-px w-auto whitespace-nowrap">
                                                <div class="flex items-center gap-2 px-6 py-2">
                                                    <span
                                                        class="font-semibold text-sm text-gray-800 dark:text-gray-200">{{ $item->transaction_id }}</span>
                                                    <button
                                                        class="text-sm text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-300 focus:outline-none"
                                            ...
                                            <td class="h-px w-auto whitespace-nowrap">
                                                <div class="px-6 py-2">
                                                    <span
                                                        class="font-semibold text-sm text-gray-800 dark:text-gray-200">{{ $item->user->name }}</span>
                                                </div>
                                            </td>
                                            <td class="h-px w-auto whitespace-nowrap">
                                                <div class="px-6 py-2">
                                                    <span class="text-sm text-gray-800 dark:text-gray-200">
                                                        Rp. {{ number_format($item->amount, 2, '.', ',') }}</span>
                                                </div>
                                            </td>
                                            <td class="h-px w-auto whitespace-nowrap">
